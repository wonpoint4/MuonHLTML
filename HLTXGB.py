...
def doXGB(seed,seedname):
    ...
    seed.drop(seed.columns[[1,3]],axis=1,inplace=True)
    ...
    param = {'max_depth':6, 'eta':0.1, 'gamma':10, 'silent':0, 'objective':'binary:logistic', 'subsample':0.5, 'eval_metric':'aucpr'}
    param['min_child_weight'] = float(fake.sum(axis=0))/10.
