...
def drawROC(fpr_Train, tpr_Train, AUC_Train, fpr_Test, tpr_Test, AUC_Test, plotname, dirname="plot"):
    ...
    plt.grid()
    plt.savefig('./'+dirname+'/'+plotname+'.png',dpi=300, bbox_inches='tight')
    if '_NThltIter2FromL1_logROC_cat1' in plotname : plotname = plotname.replace('_NThltIter2FromL1_logROC_cat1', '')
    elif '_NThltIter2_logROC_cat1' in plotname : plotname = plotname.replace('_NThltIter2_logROC_cat1', '')
    print(plotname+' : Test(Train) AUC = %.4f,(%.4f), diff = %.4f ' % (AUC_Test, AUC_Train, AUC_Test-AUC_Train))
    plt.close()
